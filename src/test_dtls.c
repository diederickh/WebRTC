...
void start_client(char *local_address, int local_port, char *remote_address, int dest_port, int length, int messagenumber) {
	int fd;
	union {
		struct sockaddr_storage ss;
		struct sockaddr_in s4;
	...
	BIO *bio;
	int reading = 0;
	int messagenumber_sent = 0;
	int messagenumber_recv = 0;
	struct timeval timeout;
...
#endif
		remote_addr.s4.sin_port = htons(dest_port);
...
#endif
		remote_addr.s6.sin6_port = htons(dest_port);
	} else {
		return;
	}
...
#endif
			local_addr.s4.sin_port = htons(local_port);
...
#endif
			local_addr.s6.sin6_port = htons(local_port);
		} else {
			return;
		}
	...
	static char* app_data = "hola qué tal œæ€å∫∂Ω∑© jeje!!!";
	int app_data_len = strlen(app_data);

	while (!(SSL_get_shutdown(ssl) & SSL_RECEIVED_SHUTDOWN)) {

		if (messagenumber_sent != messagenumber) {
			len = SSL_write(ssl, app_data, app_data_len);
		...
		if (messagenumber_sent == messagenumber) {
			// printf("all the messages sent, shutdown!\n");
			// SSL_shutdown(ssl);
		}

		reading = 1;
		while (reading) {
			len = SSL_read(ssl, buf, app_data_len);
			...
			switch (SSL_get_error(ssl, len)) {
				case SSL_ERROR_NONE:
					...
					if (len > 0)
						messagenumber_recv++;
					printf("message %d/%d received\n", messagenumber_recv, messagenumber);
					if (messagenumber_recv == messagenumber) {
						printf("all the messages received, exit\n");
						exit(0);
					}
					break;
				case SSL_ERROR_WANT_READ:
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) goto cmd_err;
  printf("argc %d\n", argc);
  //  start_client("0.0.0.0", local_port, "192.168.0.193", 59976, length, messagenumber);
	if (argc == 1)
  		start_client(local_addr, local_port, *argv, dest_port, length, messagenumber);
  else {
    printf("starting server, %s:%u.\n", local_addr, local_port);
    start_server(local_port, local_addr);
  }
